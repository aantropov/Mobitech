...
public:
    ...
    class USceneNode: public UNode
    {
        ...
        USceneNode(UGameObject* n): node(n) ,isAlreadyUpdated(false) {}

        void AddChild(USceneNode* n)
        {
    ...
    void AddLight(ULight* n)
    {
        lights.push_back(n);
    }

    vector<ULight*> GetLights()
    {
        return lights;
    }
    ...
    void KeysProccessing();

    void Update(double deltaTime);
    void UpdateLightParams();

    void Render(URENDER_TYPE type, UCamera camera);
    void Render(UMaterial *m, UCamera camera);

    UScene(void): root(NULL)
    {
